# Story 08: Swagger / OpenAPI Documentation
...
- **API**: NestJS: use `@nestjs/swagger`; apply `DocumentBuilder` and `SwaggerModule` in `main.ts`; use `@ApiTags()`, `@ApiOperation()`, `@ApiResponse()`, `@ApiBody()`, `@ApiQuery()`, `@ApiBearerAuth()` on controllers and DTOs.
- **Auth in Swagger**: Add bearer auth security scheme in `DocumentBuilder`; annotate protected routes with `@ApiBearerAuth()`.
- **Path**: e.g. `GET /docs` (UI), `GET /docs-json` (OpenAPI JSON). Disable or protect `/docs` in production if desired (e.g. behind env flag or admin role).
